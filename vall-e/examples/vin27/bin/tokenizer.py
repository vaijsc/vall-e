...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="vietnamese",
        help="Language for text extractor",
    )
    parser.add_argument(
        "--suffix",
        type=str,
...
def main():
    ...
    if args.text_extractor:
        vietnamese_north_text_tokenizer = TextTokenizer(backend=args.text_extractor, language='vi')
        vietnamese_center_text_tokenizer = TextTokenizer(backend=args.text_extractor, language='vi-vn-x-central')
        vietnamese_south_text_tokenizer = TextTokenizer(backend=args.text_extractor, language='vi-vn-x-south')
    ...
    with get_executor() as ex:
        for partition, m in manifests.items():
            ...
            if args.audio_extractor:
                ...
                if args.prefix.lower() in ["ljspeech", "aishell", "baker", "vin27"]:
            ...
            if args.text_extractor:
                if (
                    ...
                    for c in tqdm(cut_set):
                        ...
                        elif args.prefix == "vin27" or args.prefix == "mix_vin27":
                            text = c.supervisions[0].text
                            text = text.replace("”", '"').replace("“", '"')
                            if c.supervisions[0].custom["locale"] == "bac":
                                phonemes = tokenize_text(vietnamese_north_text_tokenizer, text=text)
                            elif c.supervisions[0].custom["locale"] == "trung":
                                phonemes = tokenize_text(vietnamese_center_text_tokenizer, text=text)
                            elif c.supervisions[0].custom["locale"] == "nam":
                                phonemes = tokenize_text(vietnamese_south_text_tokenizer, text=text)
                        else:
