...
def prepare_mix_vin27(
    ...
    ##sachnoi
    fr = jsonlines.open("/lustre/scratch/client/vinai/users/linhnt140/zero-shot-tts/preprocess_audio/soxan/data_stories_large_model_new_path.jsonl", "r")
    speaker_duration_dialect_dict = {}
    for line in tqdm(fr):
        if line["speaker"] not in speaker_duration_dialect_dict.keys():
            speaker_duration_dialect_dict[line["speaker"]] = {"duration": [line["duration"]], "dialect": [line["dialect"]["Label"]], "path": [line["path"]], "transcript": [line["transcript"]]}
            continue
        speaker_duration_dialect_dict[line["speaker"]]["duration"].append(line["duration"])
        speaker_duration_dialect_dict[line["speaker"]]["dialect"].append(line["dialect"]["Label"])
        speaker_duration_dialect_dict[line["speaker"]]["path"].append(line["path"])
        speaker_duration_dialect_dict[line["speaker"]]["transcript"].append(line["transcript"])
    fr.close()
    ...
    def find_most_frequent_string(strings):
        ...
        for string in strings:
            ...
            if frequency[string] > max_count:
                max_count = frequency[string]
                most_frequent = string 
        return most_frequent
    
    for speaker in speaker_duration_dialect_dict.keys():
        speakers.append(speaker)
        total_durations.append(sum(speaker_duration_dialect_dict[speaker]["duration"]))
        dialects.append(find_most_frequent_string(speaker_duration_dialect_dict[speaker]["dialect"]))

    combined = list(zip(total_durations, speakers, dialects)) 
    sorted_combined = sorted(combined, key=lambda x: x[0])
    # sorted_total_durations = [x[0] for x in sorted_combined]
    sorted_speakers = [x[1] for x in sorted_combined]
    sorted_dialects = [x[2] for x in sorted_combined]
    list_speakers_test = sorted_speakers[:20]

    for speaker in speaker_duration_dialect_dict.keys():
        for i in range(len(speaker_duration_dialect_dict[speaker]["path"])):
            audio_path = speaker_duration_dialect_dict[speaker]["path"][i]
            if not os.path.isfile(audio_path):
                logging.warning(f"No such file: {audio_path}")
                continue
            
            rec_id = speaker + "_" + "/".join(speaker_duration_dialect_dict[speaker]["path"][i].split("/")[-2:])
            print("Rec_id: ", rec_id)

            recording = Recording.from_file(speaker_duration_dialect_dict[speaker]["path"][i], recording_id=rec_id)
            if speaker_duration_dialect_dict[speaker]["dialect"][i] == "south":
                locale = "nam"
            elif speaker_duration_dialect_dict[speaker]["dialect"][i] == "center":
                locale = "trung"
            elif speaker_duration_dialect_dict[speaker]["dialect"][i] == "north":
                locale = "bac"
            segment = SupervisionSegment(
                ...
                channel=0,
                text=speaker_duration_dialect_dict[speaker]["transcript"][i],
                language="Vietnamese",
                speaker=speaker,
                gender="NA",
                custom={"orig_text": speaker_duration_dialect_dict[speaker]["transcript"][i], "locale": locale},
            )
            ...
            else: #speaker in list_speakers_train:
                recordings_train.append(recording)
                supervisions_train.append(segment)

    #########
    fw_dev = jsonlines.open("/lustre/scratch/client/vinai/users/linhnt140/zero-shot-tts/preprocess_audio/vin27_dev.jsonl", "w")
    fw_test = jsonlines.open("/lustre/scratch/client/vinai/users/linhnt140/zero-shot-tts/preprocess_audio/vin27_test.jsonl", "w")
        ...
        for speaker in tqdm(dictionary_transcription[province].keys()):
            ...
            for wav in dictionary_transcription[province][speaker].keys():
                ...
                if speaker in list_speakers_test:
                    recordings_test.append(recording)
                    supervisions_test.append(segment)
                    fw_test.write({"path": audio_path})
                elif speaker in list_speakers_dev:
                    recordings_dev.append(recording)
                    supervisions_dev.append(segment)
                    fw_dev.write({"path": audio_path})
    ...
    zipped_lists = list(zip(recordings_train, supervisions_train))
    random.shuffle(zipped_lists)
    shuffled_recordings_train, shuffled_supervisions_train = zip(*zipped_lists)

    number_of_items_per_part = int(len(shuffled_recordings_train,) / number_of_parts)
    for i in range(number_of_parts):
        st = i * number_of_items_per_part
        end = (i+1) * number_of_items_per_part
        if i != (number_of_parts - 1):
            recording_set_train = RecordingSet.from_recordings(shuffled_recordings_train[st:end])
            supervision_set_train = SupervisionSet.from_segments(shuffled_supervisions_train[st:end])
        else:
            recording_set_train = RecordingSet.from_recordings(shuffled_recordings_train[st:])
            supervision_set_train = SupervisionSet.from_segments(shuffled_supervisions_train[st:])
    ...
    if output_dir is not None:
        for i in dictionary_recordings_train.keys():
            out_record_file =  "mix_vin27_recordings_train_part_" + str(i) + ".jsonl.gz"
            out_sup_file =  "mix_vin27_supervisions_train_part_" + str(i) + ".jsonl.gz" 
        ...
        #
        supervision_set_dev.to_file(output_dir / "mix_vin27_supervisions_dev.jsonl.gz")
        recording_set_dev.to_file(output_dir / "mix_vin27_recordings_dev.jsonl.gz")
        #
        supervision_set_test.to_file(output_dir / "mix_vin27_supervisions_test.jsonl.gz")
        recording_set_test.to_file(output_dir / "mix_vin27_recordings_test.jsonl.gz")
